...
def main():
    ...
    while True:
        ...
        if key == ord('q'): break
            ...
            set_status("BI-DIRECTIONAL PROCESSING...", (0, 255, 0), duration=200)
            draw_ui(display_img, total_frames, feather_val); cv2.imshow("AI Cutout Pro (ProRes 4444)", display_img)
            cv2.waitKey(100)

            # --- BI-DIRECTIONAL AI TRACKING ---
            video_segments = {}
            ...
            for out_f_idx, _, out_mask_logits in predictor.propagate_in_video(inference_state, reverse=True):
                video_segments[out_f_idx] = (out_mask_logits[0] > 0.0).cpu().numpy().astype(np.uint8).squeeze()
            
            # --- MERGE ALPHA AND EXPORT ---
            ...
            for i in range(total_frames):
                ...
                mask = (video_segments[i] * 255).astype(np.uint8) if i in video_segments else blank_mask
            ...
            print("✨ Final Bi-Directional Cutout Ready!")
            break
