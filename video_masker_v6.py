...
def draw_ui(img, frame_count, f_idx):
    ...
    cv2.putText(img, f"Optimized Radiating Engine v6.4", (15, 55), font, 0.6, (0, 255, 255), 1)
...
def main():
    ...
    win = "Mk Masker Radiating Engine"
    ...
    while True:
        ...
        if key == ord('q'): break
            ...
            prev_start = seed_frame # Track the start of the next processed chunk
            
            for s in range(seed_frame - CHUNK_SIZE, -CHUNK_SIZE, -CHUNK_SIZE):
                real_s = max(0, s)
                real_e = prev_start # FIXED: Always ends where the previous chunk started
